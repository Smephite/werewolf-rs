...
use werewolf_rs::packet::{PacketToClient, PacketToServer, deserialize_packet, serialize_packet};
use std::env;
                ...
                tokio::spawn(async move {
                    ...
                    match ws_read.next().await {
                        Some(PacketToServer::CreateNewLobby) => {
                            lobby_send.send(lobby_manager::LobbyManagerEvent::CreateNewLobby {
                                ws_read: Box::new(ws_read),
                                ws_write: Box::new(ws_write)
                            }).await?;
                            Ok::<(), Error>(())
                        }
                        Some(PacketToServer::JoinLobby(lobby_id)) => {
                            lobby_send.send(lobby_manager::LobbyManagerEvent::JoinLobby {
                                ws_read: Box::new(ws_read),
                                ws_write: Box::new(ws_write),
                                lobby_id: lobby_id
                            }).await?;
